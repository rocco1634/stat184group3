...
```{r task1}
total_different_species <- 
  summarise(OrdwayBirds2, total_species = n_distinct(SpeciesName), index = 1)
distinct_different_species <- 
  summarise(OrdwaySpeciesNames, total_species_distinct = n_distinct(SpeciesNameCleaned, na.rm = TRUE), index = 1)

Comparison_species_count <-
  left_join(total_different_species,distinct_different_species)
Comparison_species_count %>%
  select(total_species,total_species_distinct)
```
OrdwayBirds has 275 different species including mis-spellings

OrdwaySpeciesNames has 108 distinct species names

Therefore there are 275-108 = 167 misspellings
...
* The Black-capped Chickadee and arguably the American Goldfinsh are present year round.
...
* American Goldfinch, Black-capped Chickadee, Field Sparrow, Slate-colored Junco, Tree Swallow are all present in good numbers for at least 6 months.
